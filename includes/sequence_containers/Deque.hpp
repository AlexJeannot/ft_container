...
namespace ft
{
    ...
    class Deque
    {
        ...
        public:
            ...
            Deque(size_type n, const_reference val = value_type(), const allocator_type& alloc = allocator_type())
            : _container(nullptr), _size(0), _capacity(0), _alloc(alloc)
            {
                if (n > 0)
                    this->assign(n, val);
            ...
            {
                verify_available_size(1);
                back_shift(this->begin(), 1);
                this->_alloc.construct(this->_container, val);
                this->_size++;
            }
            ...
            void pop_front(void)
            {
                if (this->_size > 0)
                {
                    //this->_alloc.destroy(this->_container);
                    front_shift(this->begin(), 1);
                    this->_size--;
                }
            }
